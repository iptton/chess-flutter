...
import 'package:testflutter/blocs/chess_events.dart';
import 'package:testflutter/blocs/chess_state.dart';
...
void main() {
  group('AI Pawn Promotion Bug Tests', () {
    ...
    test('should switch player after pawn promotion', () async {
      ...
      board[1][6] = const ChessPiece(type: PieceType.pawn, color: PieceColor.white);
      ...
      // Act: Move pawn to promotion square
      ...
      // Verify promotion dialog is shown
      expect(bloc.state.isPromotionDialogVisible, isTrue);
      expect(bloc.state.promotionPosition, equals(const Position(row: 0, col: 6)));

      // Act: Select promotion type
      ...
      expect(bloc.state.isPromotionDialogVisible, isFalse);
    ...
    test('should trigger AI move after opponent promotion in AI game', () async {
      ...
      board[6][3] = const ChessPiece(type: PieceType.pawn, color: PieceColor.black);
      ...
      // Act: Move black pawn to promotion square
      bloc.add(const MovePiece(
      ...
      // Verify promotion dialog is shown
      expect(bloc.state.isPromotionDialogVisible, isTrue);
      expect(bloc.state.promotionPosition, equals(const Position(row: 7, col: 3)));
      ...
      expect(bloc.state.isAIThinking || bloc.state.moveHistory.length > 1, isTrue);
    });
    ...
    test('should handle AI pawn promotion correctly', () async {
      ...
      board[1][2] = const ChessPiece(type: PieceType.pawn, color: PieceColor.white);
      
      // Add some other pieces to make the position more realistic
      board[6][1] = const ChessPiece(type: PieceType.pawn, color: PieceColor.black);
    ...
    test('should maintain correct turn order after multiple promotions', () async {
      ...
      board[1][1] = const ChessPiece(type: PieceType.pawn, color: PieceColor.white);
      // Place black pawn ready to promote
      board[6][6] = const ChessPiece(type: PieceType.pawn, color: PieceColor.black);
    ...
    test('should handle promotion in check situations correctly', () async {
      ...
      board[1][0] = const ChessPiece(type: PieceType.pawn, color: PieceColor.white);
      ...
      expect(bloc.state.checkedPlayer, equals(PieceColor.black));
    });
  });
}
