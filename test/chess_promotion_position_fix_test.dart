...
void main() {
  group('Chess Promotion Position Display Fix', () {
    ...
    test('测试升变过程中位置显示不出现负索引错误', () async {
      ...
      // 执行升变移动：从第6行移动到第0行
      bloc.add(MovePiece(const Position(row: 6, col: 4), const Position(row: 0, col: 4)));
      ...
      final promotionFromName = ChessFormatters.getPositionName(promotionMove.from);
    ...
    test('测试黑方升变过程中位置显示不出现负索引错误', () async {
      ...
      // 执行升变移动：从第1行移动到第7行
      bloc.add(MovePiece(const Position(row: 1, col: 3), const Position(row: 7, col: 3)));
    ...
    test('测试极端边界情况：无历史记录时的升变', () async {
      ...
      board[0][7] = const ChessPiece(type: PieceType.pawn, color: PieceColor.white);
      board[7][4] = const ChessPiece(type: PieceType.king, color: PieceColor.white);
      board[0][4] = const ChessPiece(type: PieceType.king, color: PieceColor.black);
